 ...
 * 删除照片界面
 **/
public class PhotoActivity1 extends BaseActivity {

	private PhotoActivity1 instance;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		for (int i = 0; i < Bimp1.bmp.size(); i++) {
			bmp.add(Bimp1.bmp.get(i));
		}
		for (int i = 0; i < Bimp1.drr.size(); i++) {
			drr.add(Bimp1.drr.get(i));
		}
		max = Bimp1.max;
		...
		photo_bt_enter.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				for (int i = 0; i < del.size(); i++) {
					FileUtils1.delFile(del.get(i) + ".JPEG");
				}
				Intent intent_return = new Intent(instance, YhfkActivity.class);
				startActivity(intent_return);
				overridePendingTransition(R.anim.push_right_in,

...
@SuppressLint("NewApi")
public class GeRenBanJianFragment extends BaseFragment {
	/**
	 * 个人办件Fragment
	 */
	...
	protected Object requestData() {
		...
		Log.i("GrbjFragmen缓存的数据个数", cursor.getCount()+"");
		if (flag) {
			//网络获取数据
			getData();
		}else {
	...
	private void getData() {
		...
		if (cx == true) {
			response = HttpHelper
					.get(sessionid,MainActivity.hostIp + "/webservices/Json.aspx?mod=mp&act=getinstancelist&state=yiban&pagesize=10&orgid="
...
//			Log.i("网络加载数据的list", list.size()+"");
			
			for (int i = 0; i < list.size(); i++) {
				DbsxInfo dbsxInfo = list.get(i);
//				Log.i("GeRenBanJianActivity", dbsxInfo.getId() +"     "+dbsxInfo.getStatedesc());
				mDbWrapper.insertGrbj(table, dbsxInfo);
			}
			CommonUtil.runOnUIThread(new Runnable() {
				@Override
				public void run() {
	...
	private ArrayList<DbsxInfo> selectLocalData(Cursor cursor) {
		...
		try {
			for (int i = 0; i < num; i++) {
				...
				list.add(dbsxInfo);
				Log.i("GeRenBanJianActivity", "个人办件本地缓存   ：  "+dbsxInfo.getSncode()+"");
				cursor.moveToNext();
			}

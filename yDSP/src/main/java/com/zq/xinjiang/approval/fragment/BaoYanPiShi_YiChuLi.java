...
import com.zq.xinjiang.approval.aactivity.BaoYanXiangQingActivity;
...
@SuppressLint("NewApi")
public class BaoYanPiShi_YiChuLi extends BaseFragment {
	/**
	 * 报延批示--已处理
	 */
	...
	private int num = 0;
	private final String table = "byps_ycl";
	...
	private void getData() {
		...
		try {
			...
			bypsycl_state = preferences.getInt("bypsycl_state", 0);
			
			LogUtil.recordLog("@@@@@@@@@" + bypsycl_state);
		} catch (Exception e1) {
			e1.printStackTrace();
		}
		...
		if (cx == true) {
			response = HttpHelper
					.get(sessionid,MainActivity.hostIp + "/webservices/Json.aspx?mod=mp&act=getpostponelist&state="
									+ bypsycl_state + "&pagesize=10&orgid="
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				Log.i("BaoYanPiShiActivity", dbsxInfo.getId() +"     "+dbsxInfo.getStatedesc());
				mDbWrapper.insertBypsycl(table,dbsxInfo);
			}
	...
	private ArrayList<DbsxInfo> selectLocalData(Cursor cursor) {
		...
		try {
			for (int i = 0; i < num; i++) {
				...
				dbsxInfo.setStatedesc(cursor.getString(cursor.getColumnIndex("statedesc")));

				list.add(dbsxInfo);
				Log.i("BaoYanPiShiActivity", "报延批示--已处理本地缓存   ：  "+dbsxInfo.getSncode()+"");
				cursor.moveToNext();
			}
	...
	private ArrayList<DbsxInfo> list1 = new ArrayList<DbsxInfo>();
		...
		listView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				Intent intent_bjxq = new Intent(getActivity(),
						BaoYanXiangQingActivity.class);
				intent_bjxq.putExtra("id", deitem.getIteminstanceid());
				intent_bjxq.putExtra("id_by", deitem.getId());
				editor.putString("activity", "byps");
				editor.putInt("byps_state", 2);
				editor.commit();
	...
	public class DbsxAdapter<T> extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				...
				convertView = mInflater.inflate(R.layout.activity_bypsitem,
						null);
				holder = new ViewHolder();
				/** 实例化具体的控件 */
				holder.tv_ycl_lsh = (TextView) convertView
						.findViewById(R.id.tv_ycl_lsh);
				holder.tv_ycl_zt = (TextView) convertView
						.findViewById(R.id.tv_ycl_zt);
				holder.tv_ycl_sxmc = (TextView) convertView
						.findViewById(R.id.tv_ycl_sxmc);
				holder.tv_ycl_sbr = (TextView) convertView
						.findViewById(R.id.tv_ycl_sbr);
			...
			holder.tv_ycl_lsh.setText("流水号：" + dbsxInfo.getSncode());

//			if (dbsxInfo.getStatedesc().equals("通过")) {
				holder.tv_ycl_zt.setTextColor(Color.rgb(34, 172, 56));
//			} else if (dbsxInfo.getStatedesc().equals("驳回")) {
//				holder.tv_ycl_zt.setTextColor(Color.RED);
//			}

			holder.tv_ycl_zt.setText("(" + dbsxInfo.getStatedesc() + ")");
			holder.tv_ycl_sxmc.setText("事项名称：" + dbsxInfo.getItemname());
			holder.tv_ycl_sbr.setText("申办人：" + dbsxInfo.getUsername());

...
@SuppressLint("NewApi")
public class BuMenBanJianFragmentItem extends BaseFragment {
	/**
	 * 部门办件--所有子项的Fragment	
	 */
	...
	private String table = "bmbj_thbl";
	private DBWrapper mdDbWrapper;
	private boolean flag = false;//下拉刷新
	...
	private void getData() {
		...
		if (cx == true) {
			response = HttpHelper
					.get(sessionid, MainActivity.hostIp + "/webservices/Json.aspx?mod=mp&act=getinstancelist&state="
							+ state + "&pagesize=10&orgid=" + orgid
							+ "&pageindex=" + pageindex + "&s_date="
							+ qsrq + "&e_date=" + jzrq + "&field="
							+ cx_ + "&content=" + ss);
		} else {
			response = HttpHelper
					.get(sessionid, MainActivity.hostIp + "/webservices/Json.aspx?mod=mp&act=getinstancelist&state="
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				Log.i("BuMenBanJianFragme", dbsxInfo.getId() +"     "+dbsxInfo.getSncode());
				mdDbWrapper.insertBmbj(table, dbsxInfo);
			}
			CommonUtil.runOnUIThread(new Runnable() {
				@Override
				public void run() {
	...
	protected Object requestData() {
		...
		Log.i("BmbjFragment_bm：  ", state.toString());
		
		//判断读取哪个的缓存
		if (state.equals("tuihui")) {
			table = "bmbj_thbl";
		}else if (state.equals("zuofei")) {
			table = "bmbj_zfbj";
		}else if (state.equals("zixun")) {
			table = "bmbj_zxbj";
		}else if (state.equals("bujiaobulai")) {
			table = "bmbj_bjbl";
		}else if (state.equals("shanchu")) {
			table = "bmbj_scbj";
		}else if (state.equals("banjie")) {
			table = "bmbj_zcbj";
		}else if (state.equals("zhuanbao")) {
			table = "bmbj_zbbj";
		}

		Cursor cursor = mdDbWrapper.selectTableBySpid(table, MainActivity.spid,MainActivity.hostIp);
		Log.i("BmbjFragment_bmbj", cursor.getCount()+"");
//		if (flag) {
//			//网络获取数据
//			getData();
//		}else {
//			Log.i("网络加载数据的index", list.size()/10+ "          " + pageindex);
//			if (list.size()/10<pageindex-1) {
//				getData();
//			}else {
...
//		}
	...
	private ArrayList<DbsxInfo> selectLocalData(Cursor cursor) {
		...
		try {
			for (int i = 0; i < num; i++) {
				...
				dbsxInfo.setEndtime(cursor.getString(cursor.getColumnIndex("endtime")));
				...
				list.add(dbsxInfo);
				Log.i("BuMenBanJianm", "部门办件-----本地缓存   ：  "+dbsxInfo.getSncode()+"");
				cursor.moveToNext();
			}
	...
	public class DbsxAdapter<T> extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				...
				convertView = mInflater.inflate(R.layout.activity_grybitem, null);
				...
				holder.tv_clsj = (TextView) convertView.findViewById(R.id.tv_clsj);
				holder.tv_cljg = (TextView) convertView.findViewById(R.id.tv_cljg);
			...
			holder.tv_clsj.setText("处理时间：" + dbsxInfo.getEndtime());
			holder.tv_cljg.setText(dbsxInfo.getStatedesc());
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		if (keyCode == KeyEvent.KEYCODE_BACK) {
			Intent intent_return = new Intent(getActivity(), BuMenBanJianActivity.class);

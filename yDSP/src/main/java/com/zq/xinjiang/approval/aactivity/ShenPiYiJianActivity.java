...
public class ShenPiYiJianActivity extends BaseAproActivity {
	/**
	 * 审批意见
	 */
	private ShenPiYiJianActivity instance;
	private ImageView return_main;
	private TextView actionBarReturnText;
	private EditText et_spyj;
	private LinearLayout line_ty, line_th;
	...
	private String id, loginname, sncode, flowid, stepid;
	...
	private String itemname, username, orgname, sncode_, linkmobile, address,
			charge, limittime;
...
//		getWindow().setFeatureInt(Window.FEATURE_CUSTOM_TITLE, R.layout.activity_titlebar);
		...
		LogUtil.recordLog("^^^^^^^^^^^^^^^^：" + id);

		loginname = preferences.getString("loginname", "");
		sncode = this.getIntent().getExtras().getString("sncode");
		flowid = this.getIntent().getExtras().getString("flowid");
		stepid = this.getIntent().getExtras().getString("stepid");
...
//		actionBarReturnText = (TextView) findViewById(R.id.actionBarReturnText);
		...
		line_ty = (LinearLayout) findViewById(R.id.line_ty);
		line_th = (LinearLayout) findViewById(R.id.line_th);
...
//		actionBarReturnText.setText("审批意见");

		return_main.setOnClickListener(listener);
		line_ty.setOnClickListener(listener);
		line_th.setOnClickListener(listener);
	...
	private OnClickListener listener = new OnClickListener() {
		public void onClick(View v) {
			...
			case R.id.return_main:
				Intent intent_return = new Intent(instance, DaiBanShiXiangActivity.class);
			...
			case R.id.line_ty:
				...
				Config.timerTask(line_ty,3000);
				spyj = et_spyj.getText().toString().trim();
				if ("".equals(spyj) || "null".equals(spyj)) {
					initToast("审批意见不能为空！");
				} else {
					sptg();
...
//				spth();
				...
				poup_tv.setText("本件已退回！");
				break;
			case R.id.popup_submit:
				popWindow.dismiss();
				Intent intent_submit = new Intent(instance, DaiBanShiXiangActivity.class);
	...
	private void sptg() {
		spyj = et_spyj.getText().toString().trim();
		String sptgURL =  MainActivity.hostIp + "/webservices/Json.aspx?mod=mp&act=instanceofnextstep&instance="
				+ id + "&flow=" + flowid + "&remark=" + spyj + "&type=Shenhe" +
				// "&loginname=" + loginname +
				"&stepid=" + stepid;
		LogUtil.recordLog("审批通过地址：" + sptgURL);
		if (MSimpleHttpUtil.isCheckNet(ShenPiYiJianActivity.this)) {

			Header[] reqHeaders = new BasicHeader[1];
			reqHeaders[0] = new BasicHeader("cookie", "ASP.NET_SessionId=" + sessionid);

			finalHttp.get(sptgURL, reqHeaders, null, new AjaxCallBack<String>() {
...
//									}
	...
	private void spth() {
		String spthURL =  MainActivity.hostIp + "/webservices/Json.aspx?mod=mp&act=updatetuiban&instance="
				+ id + "&flow=" + flowid
				// + "&loginname=" + loginname
				+ "&sncode=" + sncode;
		LogUtil.recordLog("审批退回地址：" + spthURL);
		if (MSimpleHttpUtil.isCheckNet(ShenPiYiJianActivity.this)) {

			Header[] reqHeaders = new BasicHeader[1];
			reqHeaders[0] = new BasicHeader("cookie", "ASP.NET_SessionId=" + sessionid);

			finalHttp.get(spthURL, reqHeaders, null,
						...
						public void onSuccess(String t) {
							...
							try {
								...
								if (errno == 0) {
									...
									sncode_ = jsonitem.getString("sncode");
									...
									if (jsonArray_docs.length() == 0) {
										...
										ma = new MoreAdapter(ShenPiYiJianActivity.this, attentionList);
									...
									if (jsonArray.length() == 0) {
										...
										for (int j = 0; j < jsonArray.length(); j++) {
											...
											if (jsonArray_logs.length() == 0) {
												...
												// if(bjxxxx.getLogs_stepname().equals("开始")||bjxxxx.getLogs_stepname().equals("窗口受理")||bjxxxx.getLogs_stepname().equals("部门批示")||
												// bjxxxx.getLogs_stepname().equals("独立收费")){
												// poup_tv.setText("流程结束！");
												// }else
												// if(bjxxxx.getLogs_stepname().equals("窗口签发")){
												// poup_tv.setText("审批完成！");
												// }
												//
											}
...
//									}
	...
	public class MoreAdapter extends BaseAdapter {
		...
		public MoreAdapter(ShenPiYiJianActivity iccnewsActivity, ArrayList<BjxxxxEntity> attentionList) {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			holder.line_xz.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View arg0) {
					...
					// LogUtil.recordLog("文件下载地址222：" + url + "-----" +
					// wj_name);
	...
	private void wjxz() {
		...
		if (MSimpleHttpUtil.isCheckNet(ShenPiYiJianActivity.this)) {
			// dialog = ProgressDialog.show(ShenPiYiJianActivity.this, null,
			// "正在下载中...");
	...
	Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			if (progress == 100) {
				...
				dialog = ProgressDialog.show(ShenPiYiJianActivity.this, null, "正在加载中...");
				dialog.setCancelable(true);
			}
		}
	};

...
import com.zq.xinjiang.approval.aactivity.DuBanActivity;
...
@SuppressLint("NewApi")
public class YiChangBanJianFragment_GuoQi extends BaseFragment {
	/**
	 * 异常办件--过期
	 */
	...
	private String db_id, db_flowid, actorid, actorname;
	Calendar calendar;
	...
	private int num = 0;
	private final String table = "ycbj_gq";
...
//		if (cx == true) {
//			response = HttpHelper
//					.get(sessionid,MainActivity.hostIp + "/webservices/Json.aspx?mod=mp&act=getinstancelist&state=guoqi&pagesize=10&orgid="
...
//		}
		...
		if (cx == true) {
			response = HttpHelper
					.get(sessionid, MainActivity.hostIp + "/webservices/Json.aspx?state=guoqi&mod=mp&act=gethandlelist&status=zaiban&pageindex="
							+ pageindex
							+ "&s_date=" + qsrq + "&e_date=" + jzrq
							+ "&field=" + cx_ + "&content=" + ss);
		} else {
			response = HttpHelper
					.get(sessionid,MainActivity.hostIp + "/webservices/Json.aspx?s_date=1438594140000&e_date="+currentTime+"&pageindex="+ pageindex+
							"&pagesize=10&field=itemname&content=&state=guoqi&mod=mp&act=gethandlelist&status=zaiban");
		}
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				mDbWrapper.insertYcbjgq(table, dbsxInfo);
			}
...
//			});
	...
	protected Object requestData() {
		...
		Log.i("YcbjFragmen缓存的数据个数", cursor.getCount()+"");
		if (flag) {
			//网络获取数据
			getData();
		}else {
	...
	private ArrayList<DbsxInfo> selectLocalData(Cursor cursor) {
		...
		try {
			for (int i = 0; i < num; i++) {
				...
				list.add(dbsxInfo);
				Log.i("YiChangBanJianActivity", "异常办件--过期本地缓存   ：  "+dbsxInfo.getSncode()+"");
				cursor.moveToNext();
			}
...
//				listView.setAdapter(dbsxAdapter);
	...
	private void initPullToRefreshListView() {
		...
		listView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
				...
				editor.putString("activity", "ycbj");
				editor.putInt("ycbj_state", 1);
				editor.commit();
	...
	private void dbhqdqxx() {
		String dbhqdqxxURL = MainActivity.hostIp + "/webservices/Json.aspx?mod=mp&act=getworkflowtasks&active=true&instanceid="
				+ db_id + "&flow=" + db_flowid;
		LogUtil.recordLog("督办获取当前信息地址：" + dbhqdqxxURL);
		if (MSimpleHttpUtil.isCheckNet(getActivity())) {
			...
			finalHttp.get(dbhqdqxxURL, reqHeaders, null, new AjaxCallBack<String>() {
							...
							try {
								...
								if (errno == 0) {
									...
									if (jsonArray.length() == 0) {
										...
										JSONObject jsonOrder_items = jsonArray.getJSONObject(0);
										if (jsonOrder_items.length() == 0) {

										} else {
											JSONArray jsonArray_tasks = jsonOrder_items.getJSONArray("tasks");
											if (jsonArray_tasks.length() == 0) {

											} else {
												JSONObject jsonOrder_tasks = jsonArray_tasks.getJSONObject(0);
												actorid = jsonOrder_tasks.getString("actorid");
												actorname = jsonOrder_tasks.getString("actorname");
												LogUtil.recordLog("actorid：" + actorid);

												editor.putString("actorid", actorid);
												editor.putString("actorname", actorname);
												editor.commit();
											}
										}
									}
			...
			CommonUtil.runOnUIThread(new Runnable() {
				@Override
				public void run() {
					Toast.makeText(getActivity(),R.string.network_anomaly,Toast.LENGTH_SHORT).show();
				}
			});		}
	}
	...
	public class DbsxAdapter<T> extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				...
				convertView = mInflater.inflate(R.layout.activity_ycbjitem, null);
			...
			holder.line_xyb.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View arg0) {
					...
					db_id = dbsx.getId();
					db_flowid = dbsx.getFlowid();

					dbhqdqxx();

					Intent intent_ycbj = new Intent(getActivity(), DuBanActivity.class);
					editor.putInt("dbsx_state", 1);
					editor.commit();
					intent_ycbj.putExtra("db_id", dbsx.getId());
					intent_ycbj.putExtra("db_flowid", dbsx.getFlowid());
					intent_ycbj.putExtra("sncode", dbsx.getSncode());
					intent_ycbj.putExtra("itemname", dbsx.getItemname());

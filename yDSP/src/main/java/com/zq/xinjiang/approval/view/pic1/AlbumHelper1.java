...
public class AlbumHelper1 {
	...
	HashMap<String, ImageBucket1> bucketList = new HashMap<String, ImageBucket1>();
	...
	void buildImagesBucketList() {
		...
		Iterator<Entry<String, ImageBucket1>> itr = bucketList.entrySet()
				.iterator();
		while (itr.hasNext()) {
			Map.Entry<String, ImageBucket1> entry = (Map.Entry<String, ImageBucket1>) itr
					.next();
			ImageBucket1 bucket = entry.getValue();
			Log.d(TAG, entry.getKey() + ", " + bucket.bucketName + ", "
					+ bucket.count + " ---------- ");
			for (int i = 0; i < bucket.imageList.size(); ++i) {
				ImageItem1 image = bucket.imageList.get(i);
	 ...
	 * @return
	 */
	public List<ImageBucket1> getImagesBucketList(boolean refresh) {
		if (refresh || (!refresh && !hasBuildImagesBucketList)) {
			buildImagesBucketList();
		}
		List<ImageBucket1> tmpList = new ArrayList<ImageBucket1>();
		Iterator<Entry<String, ImageBucket1>> itr = bucketList.entrySet()
				.iterator();
		while (itr.hasNext()) {
			Map.Entry<String, ImageBucket1> entry = (Map.Entry<String, ImageBucket1>) itr

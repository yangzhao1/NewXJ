...
@SuppressLint("NewApi")
public class DaiBanShiXiang_ZhengChang extends BaseFragment {
	/**
	 * 待办事项--正常
	 */
...
//		if (cx == true) {
//			response = HttpHelper
//					.get(sessionid,MainActivity.hostIp + "/webservices/Json.aspx?mod=mp&act=getinstancelist&state=zhengchang&pagesize=10&orgid="
...
//		}
		...
		if (cx == true) {
			response = HttpHelper
					.get(sessionid, MainActivity.hostIp + "/webservices/Json.aspx?state=zhengchang&mod=mp&act=gethandlelist&status=zaiban&pageindex="
							+ pageindex
							+ "&s_date=" + qsrq + "&e_date=" + jzrq
							+ "&field=" + cx_ + "&content=" + ss);
		} else {
			response = HttpHelper
					.get(sessionid,MainActivity.hostIp + "/webservices/Json.aspx?s_date=1438594140000&e_date="+ currentTime +"&pageindex="+ pageindex+
							"&pagesize=10&field=itemname&content=&state=zhengchang&mod=mp&act=gethandlelist&status=zaiban");
		}
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				mdDbWrapper.insertDbsxzc(table, dbsxInfo);
			}
			CommonUtil.runOnUIThread(new Runnable() {
				@Override
				public void run() {
	...
	private ArrayList<DbsxInfo> selectLocalData(Cursor cursor) {
		...
		try {
			for (int i = 0; i < num; i++) {
				...
				list.add(dbsxInfo);
				Log.i("DaiBanShiXiangActivity", "异常办件--正常本地缓存   ：  "+dbsxInfo.getSncode()+"");
				cursor.moveToNext();
			}

...
import com.zq.xinjiang.approval.homeactivity.ShouFeiFenXiActivity;
...
public class SflbActivity extends BaseAproActivity {

	private SflbActivity instance;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_sflb);
		// 设置标题为某个layout
	...
	private void initView() {
		...
		actionBarReturnText.setText("收费列表");
	...
	private OnClickListener listener = new OnClickListener() {
		public void onClick(View v) {
			...
			case R.id.return_main:
				Intent intent_return = new Intent(instance, ShouFeiFenXiActivity.class);
			...
			case R.id.line_qsrq:
				...
				SflbActivity.this.saleHandler.sendMessage(msg);
				break;
			case R.id.line_jzrq:
				start_or_stop = 2;
				Message msg1 = new Message();
				msg1.what = instance.SHOW_DATAPICK;
				SflbActivity.this.saleHandler.sendMessage(msg1);
				break;
			case R.id.btn_ss:
				pageindex = 1;
				sflbss();
	...
	Handler saleHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			switch (msg.what) {
			case SflbActivity.SHOW_DATAPICK:
	...
	private void sflb() {
		String sflbURL = ip+":"
				+ dk+"/"+zd
				+ "/webservices/Json.aspx?mod=mp&act=getchargelist&pagesize=20&pageindex="
				+ pageindex + "&orgid=" + orgid;
		LogUtil.recordLog("收费列表地址：" + sflbURL);
		if (MSimpleHttpUtil.isCheckNet(SflbActivity.this)) {
			dialog = ProgressDialog.show(SflbActivity.this, null, "正在加载中...");
			...
			finalHttp.get(sflbURL, reqHeaders, null,
							...
							if (jsonArray.length() == 0) {
								...
								for (int i = 0; i < jsonArray.length(); i++) {
									...
									dbsx.setUsername(jsonOrder
											.getString("username"));
									dbsx.setCharge(jsonOrder
											.getString("charge"));

									attentionList.add(dbsx);
								}
								ma = new MoreAdapter(SflbActivity.this,
										attentionList);
								...
								list.setOnItemClickListener(new OnItemClickListener() {
									...
									public void onItemClick(
										...
										deitem = (DbsxEntity) attentionList
												.get(arg2);
										Intent intent_sflb = new Intent(
												instance, BanJianXiangQingActivity.class);
										intent_sflb.putExtra("id",
												deitem.getId());
										editor.putString("activity", "sflb");
										editor.commit();
										startActivity(intent_sflb);
...
//		jzrq = tv_jzrq.getText().toString().trim();
		...
		String sflbURL = ip+":"
				+ dk+"/"+zd
				+ "/webservices/Json.aspx?mod=mp&act=getchargelist&pagesize=20&pageindex="
				+ pageindex + "&orgid=" + orgid + "&s_date=" + qsrq
				+ "&e_date=" + jzrq + "&field=" + cx_ + "&content=" + ss;
		LogUtil.recordLog("收费列表地址：" + sflbURL);
		if (MSimpleHttpUtil.isCheckNet(SflbActivity.this)) {
			dialog = ProgressDialog.show(SflbActivity.this, null, "正在加载中...");
			dialog.setCancelable(true);
...
//				line_greyxyy.setVisibility(View.VISIBLE);
	...
	public class MoreAdapter extends BaseAdapter {
		...
		public MoreAdapter(SflbActivity iccnewsActivity,
				ArrayList<DbsxEntity> attentionList) {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			holder.tv_sfje.setText("¥" + dbsxEntity.getCharge());

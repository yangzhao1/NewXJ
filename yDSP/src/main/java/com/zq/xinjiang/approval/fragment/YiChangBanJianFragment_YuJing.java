...
@SuppressLint("NewApi")
public class YiChangBanJianFragment_YuJing extends BaseFragment {
	/**
	 * 异常办件--预警
	 */
	...
	private int num = 0;
	private final String table = "ycbj_yj";
	...
	private void initView() {
		...
		sp.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
				String[] languages = getResources().getStringArray(R.array.list1);
				cx_ = languages[arg2];
				et_ss.setText("");
			}
...
//		}
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				mDbWrapper.insertYcbjyj(table, dbsxInfo);
			}
	...
	private ArrayList<DbsxInfo> selectLocalData(Cursor cursor) {
		...
		try {
			for (int i = 0; i < num; i++) {
				...
				list.add(dbsxInfo);
				Log.i("YiChangBanJianActivity", "异常办件--预警本地缓存   ：  "+dbsxInfo.getSncode()+"");
				cursor.moveToNext();
			}
	...
	private void dbhqdqxx() {
		...
		if (MSimpleHttpUtil.isCheckNet(getActivity())) {
			...
			CommonUtil.runOnUIThread(new Runnable() {
				@Override
				public void run() {
					Toast.makeText(getActivity().getApplicationContext(),R.string.network_anomaly,Toast.LENGTH_SHORT).show();
				}
			});		}
	}

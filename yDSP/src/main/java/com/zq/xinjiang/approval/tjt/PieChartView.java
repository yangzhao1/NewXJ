...
public class PieChartView extends View {
	...
	private int[] mPieColors = new int[] { Color.rgb(180, 198, 236),
			Color.rgb(252, 176, 64), Color.rgb(238, 233, 109),
			Color.rgb(255, 129, 90), Color.rgb(75, 219, 204),
			Color.rgb(253, 217, 98), Color.rgb(243, 194, 162),
			Color.rgb(255, 143, 144), Color.rgb(183, 234, 45),
			Color.rgb(255, 181, 178) };
		...
		if (mPieItems != null && mPieItems.length > 0) {
			...
			for (int i = 0; i < mPieItems.length; i++) {
				...
				if (sweep != 0.0) {
					...
					// If the item is very small, make the text further away
					// from
					...
					lineStopY = pieCenterY + pieRadius * rate
							* (float) (Math.sin(radians));
					canvas.drawLine(lineStartX, lineStartY, lineStopX,
							lineStopY, linePaint);

					// write text
					String itemTypeText = mPieItems[i].getItemType();
					String itemPercentText = Utility.formatFloat(mPieItems[i]
					...
					if (lineStartX > pieCenterX) {
						textStartX += (smallMargin + Math.abs(itemTypeTextLen
								- lineTextWidth) / 2);
						percentStartX += (smallMargin + Math
					...
					canvas.drawText(itemTypeText, textStartX, textStartY,
							textPaint);
					// draw percent text
					canvas.drawText(itemPercentText, percentStartX,
							percentStartY, textPaint);
					...
					canvas.drawLine(lineStopX, lineStopY, textLineStopX,
							lineStopY, linePaint);

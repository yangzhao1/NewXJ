...
import com.zq.xinjiang.approval.activity.BaoYanChuLiActivity;
...
@SuppressLint("NewApi")
public class BaoYanPiShi_DaiChuLi extends BaseFragment {
	/**
	 * 报延批示--待处理
	 */
	...
	private int num = 0;
	private final String table = "byps_dcl";
	...
	private void getData() {
		...
		if (cx == true) {
			response = HttpHelper
					.get(sessionid, MainActivity.hostIp + "/webservices/Json.aspx?mod=mp&act=getpostponelist&state=0&pagesize=10&orgid="
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				mDbWrapper.insertBypsdcl(table,dbsxInfo);
			}
			CommonUtil.runOnUIThread(new Runnable() {
				@Override
				public void run() {
	...
	private ArrayList<DbsxInfo> selectLocalData(Cursor cursor) {
		...
		try {
			for (int i = 0; i < num; i++) {
				...
				list.add(dbsxInfo);
				Log.i("BaoYanPiShiActivity", "报延批示--待处理本地缓存   ：  "+dbsxInfo.getSncode()+"");
				cursor.moveToNext();
			}
	...
	public class DbsxAdapter<T> extends BaseAdapter {
		...
		class ViewHolder {
			...
			TextView tv_ycl_sbr;
			LinearLayout line_cl;
			ImageView image_jbxx_y;
		}
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				...
				convertView = mInflater.inflate(R.layout.activity_bypsitem, null);
				holder = new ViewHolder();
				/** 实例化具体的控件 */
				holder.tv_ycl_lsh = (TextView) convertView.findViewById(R.id.tv_ycl_lsh);
				holder.tv_ycl_sxmc = (TextView) convertView.findViewById(R.id.tv_ycl_sxmc);
				holder.tv_ycl_sbr = (TextView) convertView.findViewById(R.id.tv_ycl_sbr);
				holder.image_jbxx_y = (ImageView) convertView.findViewById(R.id.image_jbxx_y);
				holder.line_cl = (LinearLayout) convertView.findViewById(R.id.line_cl);
			...
			holder.image_jbxx_y.setVisibility(View.GONE);
			holder.line_cl.setVisibility(View.VISIBLE);

			holder.line_cl.setTag(dbsxInfo);

			holder.line_cl.setOnClickListener(new OnClickListener() {

				@Override
				public void onClick(View arg0) {
					DbsxInfo dbsx = (DbsxInfo) arg0.getTag();

					Intent intent_yps = new Intent(getActivity(), BaoYanChuLiActivity.class);
					intent_yps.putExtra("id", dbsx.getId());
					startActivity(intent_yps);
					getActivity().finish();
				}
			});

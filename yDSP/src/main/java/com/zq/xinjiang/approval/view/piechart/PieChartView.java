...
//	private float triggerX, triggerY;
	...
	// 默认颜色
//	private static final String[] DEFAULT_ITEMS_COLORS = { "#6A5ACD",
...
//			"#006400", "#00FFFF", "#00FFFF", "#668B8B", "#000080", "#008B8B" };
	private static final String[] DEFAULT_ITEMS_COLORS = { "#b4c6ec",
		"#fcb040", "#eee96d", "#ff815a", "#4bdbcc", "#fdd962", "#f3c2a2",
		"#ff8f90", "#b7ea2d", "#ffb5b2", "#FF00FF", "#FF83FA", "#0000FF",
		"#363636", "#FFDAB9", "#90EE90", "#8B008B", "#00BFFF", "#00FF00",
		"#006400", "#00FFFF", "#00FFFF", "#668B8B", "#000080", "#008B8B" };
...
//				paint.setStyle(Paint.Style.FILL); 
			...
			canvas.drawCircle(centerX , centerY, radius/4+context.getResources().getDimension(R.dimen.statistics_circle_radius), p1) ;
			p1.setColor(Color.WHITE) ;
			canvas.drawCircle(centerX , centerY, radius/4, p1) ;
//			drawImg(canvas, centerX, centerY, radius/2);
	...
	private void drawText(Canvas canvas, float x, float y, int radius) {
		...
		textPaint.setTextSize(context.getResources().getDimension(R.dimen.statistics_in_circle_txt));
...
//					}else 
						if((getWidth()/2 - x1)*(getWidth()/2 - x1)+(getHeight()/2 - y1)*(getHeight()/2 - y1) - (radius/2)*(radius/2)<=0){
						//点击了圆内空白区域也触发
						notifyTriggerClickListeners();
					}else{
						// 在点击了扇形

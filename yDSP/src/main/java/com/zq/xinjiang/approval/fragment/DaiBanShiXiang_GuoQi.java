...
@SuppressLint("NewApi")
public class DaiBanShiXiang_GuoQi extends BaseFragment {
	/**
	 * 待办事项--过期
	 */
	...
	private String qsrq = "1438594140000";
	private String jzrq = "1477843200000";
...
//		LogUtil.recordLog("当前时间是： "+ currentTime + "  "+calendar.get(Calendar.YEAR)+"  "+calendar.get(Calendar.MONTH)+"  "+calendar.get(Calendar.DAY_OF_MONTH));
...
//		http://192.168.1.104/sp/wq/webservices/Json.aspx?s_date=1438594140000&e_date=1477843200000&pageindex=1&pagesize=10
		// &field=itemname&content=&state=guoqi&mod=mp&act=gethandlelist&status=zaiban
...
//		}
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				Log.i("DaiBanShiXiangActivity1", dbsxInfo.getId() +"     "+dbsxInfo.getSncode());
				mdDbWrapper.insertDbsxgq(table, dbsxInfo);
			}
			CommonUtil.runOnUIThread(new Runnable() {
				@Override
				public void run() {
	...
	private void getData2() {
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				DbsxInfo dbsxInfo = list.get(i);
				Log.i("DaiBanShiXiangActivity2", dbsxInfo.getId() +"     "+dbsxInfo.getSncode());
				mdDbWrapper.insertDbsxgq(table, dbsxInfo);
			}
			CommonUtil.runOnUIThread(new Runnable() {
				@Override
	...
	private ArrayList<DbsxInfo> selectLocalData(Cursor cursor) {
		list.clear();
		LogUtil.recordLog("查询本地数据数量：" +  num);
		cursor.moveToFirst();
		try {
			for (int i = 0; i < num; i++) {
				...
				list.add(dbsxInfo);
				Log.i("DaiBanShiXiangActivity", "异常办件--过期本地缓存   ：  "+dbsxInfo.getSncode()+"");
				cursor.moveToNext();
			}
